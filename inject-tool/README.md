...
| cursor-cli | bundle | Cursor Agent CLI |
...
**Bundle tools** (kilocode, gemini-cli, cursor-cli): patches the DevWorkspace CR to add an init container that copies the full Node.js runtime + tool directory to `/injected-tools/<tool>/`. The editor container gets a +512Mi memory bump.
